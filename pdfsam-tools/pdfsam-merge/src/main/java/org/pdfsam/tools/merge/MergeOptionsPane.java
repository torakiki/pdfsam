...
class MergeOptionsPane extends VBox
    ...
    private final CheckBox normalize;
    ...
    MergeOptionsPane() {
        ...
        normalize = new CheckBox(i18n().tr("Normalize pages size"));
        normalize.setGraphic(helpIcon(i18n().tr("Resizes all pages to have the same width as the first page.")));
        ...
        acroForms.getItems().add(
                new ComboItem<>(AcroFormPolicy.MERGE_RENAMING_EXISTING_FIELDS, i18n().tr("Merge renaming existing fields")));
        ...
        options.add(new Label(i18n().tr("Interactive forms (AcroForms):")), 0, 0);
        acroForms.setMaxWidth(Double.POSITIVE_INFINITY);
        options.add(acroForms, 1, 0);
        options.add(helpIcon(i18n().tr("What to do in case one or more input documents contain Acro Forms")), 2, 0);
        ...
        outline.getItems().add(
                new ComboItem<>(OutlinePolicy.ONE_ENTRY_EACH_DOC, i18n().tr("Create one entry for each merged document")));
        ...
        options.add(new Label(i18n().tr("Bookmarks handling:")), 0, 1);
        outline.setMaxWidth(Double.POSITIVE_INFINITY);
        options.add(outline, 1, 1);
        options.add(helpIcon(i18n().tr("What to do in case one or more input documents contain bookmarks")), 2, 1);
        ...
        toc.setId("tocCombo");
        options.add(new Label(i18n().tr("Table of contents:")), 0, 2);
    ...
    public void restoreStateFrom(Map<String, String> data) {
        ...
        normalize.setSelected(Boolean.parseBoolean(data.get("normalize")));
    }
}
